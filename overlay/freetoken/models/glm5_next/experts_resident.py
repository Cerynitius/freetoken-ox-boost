...
class ResidentNvfp4Experts(BaseOP):
    ...
    def routed_forward(
        ...
        # Routing-trace hook (resident layers), tagged 1000+checkpoint-layer so the
        # analyzer can tell them from offload bank ids. Eager measurement only.
        from freetoken.layers.moe import _ROUTE_TRACE, _ROUTE_TRACE_BUF

        if _ROUTE_TRACE:
            import torch as _t

            _ROUTE_TRACE_BUF.append((1000 + self.layer_id,
                                     "P" if get_global_ctx().batch.is_prefill else "D",
                                     topk_ids.detach().cpu().clone(),
                                     hidden_states.detach().to(_t.float16).cpu().clone()))
