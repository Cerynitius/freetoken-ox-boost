...
def _vision_cfg_dict(hf_config) -> dict | None:
    ...
    default OFF keeps the production text-only boot byte-identical."""
    if not _VISION:
        return None
...
def parse_config(hf_config: Any) -> ModelConfig:
    ...
    return ModelConfig(
        ...
        # Serving knob: cap routed experts per token (FREETOKEN_GLM5_TOPK, default =
        # checkpoint top-8). Decode miss bytes scale linearly with it; quality cost is
        # the dropped lowest-weight experts (A/B-gated before any default change).
        num_experts_per_tok=min(
            a.num_experts_per_tok,
            int(os.environ.get("FREETOKEN_GLM5_TOPK", "0") or a.num_experts_per_tok),
        ) if int(os.environ.get("FREETOKEN_GLM5_TOPK", "0") or 0) > 0 else a.num_experts_per_tok,
